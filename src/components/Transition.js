...
const TransitionContainer = class extends React.Component {
  ...
  render() {
    ...
    if(transition.type === transitions.enter.type) {
    	return (<div className={`transition-group ${className} ${transition.type} ${enter.join(' ')}`}>{children}</div>)	
    } else {
    	return (<div className={`transition-group ${className} ${transition.type} ${exit.join(' ')}`}>{children}</div>)	
    }
  }
}
